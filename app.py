...
# Звания в клане
CLAN_RANK_VASSAL = 'vassal'
CLAN_RANK_KNIGHT = 'knight'
CLAN_RANK_BARON = 'baron'
CLAN_RANK_COUNT = 'count'
CLAN_RANK_MARQUIS = 'marquis'
CLAN_RANK_DUKE = 'duke'

CLAN_RANK_TITLES = {
    CLAN_RANK_VASSAL: 'Вассал',
    CLAN_RANK_KNIGHT: 'Рыцарь',
    CLAN_RANK_BARON: 'Барон',
    CLAN_RANK_COUNT: 'Граф',
    CLAN_RANK_MARQUIS: 'Маркиз',
    CLAN_RANK_DUKE: 'Герцог',
}
...
DEMOGORGON_DAMAGE_PER_TICK = 150  # урон по силе области за один тик
...
class User(UserMixin, db.Model):
    ...
    @property
    def clan_title(self):
        """Человекочитаемое звание пользователя в клановой системе."""
        # Пользователь без клана
        if not self.clan_id:
            return 'Бродяга'
        # Владелец клана
        if self.clan_obj and self.id == self.clan_obj.owner_id:
            return 'Великий князь'
        # Обычные звания
        rank = self.clan_rank or CLAN_RANK_VASSAL
        return CLAN_RANK_TITLES.get(rank, CLAN_RANK_TITLES[CLAN_RANK_VASSAL])
...
def cabinet():
    ...
    if clan:
        ...
        def _clan_can_accept_requests(u, c):
            if not c or u.clan_id != c.id:
                return False
            if c.owner_id == u.id:
                return True
            return (u.clan_rank or CLAN_RANK_VASSAL) in (CLAN_RANK_DUKE, CLAN_RANK_MARQUIS)

        def _clan_can_kick_members(u, c):
            if not c or u.clan_id != c.id:
                return False
            if c.owner_id == u.id:
                return True
            return (u.clan_rank or CLAN_RANK_VASSAL) == CLAN_RANK_DUKE

        can_accept = _clan_can_accept_requests(user, clan)
        can_kick = _clan_can_kick_members(user, clan)
        ...
        clan_data = {
            ...
            'members': [{
                ...
                'rank': u.clan_rank,
                'title': u.clan_title
            } for u in clan.members_rel],
            'territories': [{
                'region_index': t.region_index,
                'display_name': regions_cfg.get(t.region_index, f'Зона {t.region_index+1}'),
                'strength': t.strength
            ...
            'pending_requests': [{
                ...
                'character_name': r.user.character_name or r.user.username,
                'message': r.message
            } for r in clan.join_requests if r.status == 'pending'] if can_accept else [],
...
def api_clan_accept_request(clan_id, request_id):
    """Принять заявку (владелец, Герцог или Маркиз клана)"""
    clan = Clan.query.get_or_404(clan_id)
    user = current_user
...
def api_clan_reject_request(clan_id, request_id):
    """Отклонить заявку (владелец, Герцог или Маркиз клана)"""
    clan = Clan.query.get_or_404(clan_id)
    user = current_user
    if not user.clan_id or user.clan_id != clan.id:
        return jsonify({'success': False, 'error': 'Вы не состоите в этом клане'}), 403
    if clan.owner_id != user.id and (user.clan_rank or CLAN_RANK_VASSAL) not in (CLAN_RANK_DUKE, CLAN_RANK_MARQUIS):
        return jsonify({'success': False, 'error': 'Недостаточно прав для отклонения заявок'}), 403
    req = ClanJoinRequest.query.filter_by(id=request_id, clan_id=clan_id, status='pending').first()
    if not req:
        return jsonify({'success': False, 'error': 'Заявка не найдена'}), 404
    req.status = 'rejected'
...
def api_clan_kick(clan_id, user_id):
    ...
    if not user.clan_id or user.clan_id != clan.id:
        return jsonify({'success': False, 'error': 'Вы не состоите в этом клане'}), 403
    if clan.owner_id != user.id and (user.clan_rank or CLAN_RANK_VASSAL) != CLAN_RANK_DUKE:
        return jsonify({'success': False, 'error': 'Недостаточно прав для исключения участников'}), 403
...
def api_clan_transfer_ownership(clan_id):
    ...
    # Старый владелец теряет особый статус владения; если у него не было явного звания, назначим Вассала.
    old_owner = current_user
    if not old_owner.clan_rank:
        old_owner.clan_rank = CLAN_RANK_VASSAL
    # Новый владелец получает статус Великий князь, звание определяется логикой clan_title (без необходимости явно задавать clan_rank).
...
@login_required
def api_clan_set_rank(clan_id, user_id):
    """Изменить звание участника клана (только создатель клана)."""
    clan = Clan.query.get_or_404(clan_id)
    if clan.owner_id != current_user.id:
        return jsonify({'success': False, 'error': 'Только создатель клана может изменять звания'}), 403
    target = User.query.get_or_404(user_id)
    if target.clan_id != clan_id:
        return jsonify({'success': False, 'error': 'Пользователь не состоит в этом клане'}), 400
    if target.id == clan.owner_id:
        return jsonify({'success': False, 'error': 'Нельзя изменить звание владельца клана'}), 400
    data = request.get_json() or {}
