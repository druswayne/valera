    ...
    def _eval(node: ast.AST) -> int:
        ...
        if isinstance(node, ast.BinOp):
            ...
            if isinstance(node.op, ast.Pow):
                # Степень допускаем только от 0 до 5 (включительно),
                # чтобы в условиях нигде не появлялись показатели > 5.
                if right < 0 or right > 5:
...
def generate_territory_percent_task(difficulty: int) -> Dict[str, Any]:
    ...
    if difficulty == 1:
        ...
        if kind == "pct_of_number":
            ...
            ans = N * p // 100
            desc = random.choice([
                f"Найдите {p}% от числа {N}. В ответ укажите одно число.",
                f"Сколько будет составлять {p}% от {N}? В ответ укажите одно число.",
                f"Число равно {N}. Найдите {p}% этого числа. В ответ укажите одно число.",
        ...
        for _ in range(3):
            ...
            templates.append({
                "desc": random.choice([
                    ...
                    f"На счёт положили {N} рублей. Начислили {p}% годовых. Сколько рублей составят проценты за год? В ответ укажите одно число.",
                    f"Турист прошёл {N} км. {p}% пути он шёл по лесу. Сколько километров он прошёл по лесу? В ответ укажите одно число.",
                ]),
                "ans": str(ans),
            })
            # 2) цена после скидки
            ...
            if final_price > 0:
                templates.append({
                    "desc": random.choice([
                        f"Телефон стоит {N} рублей. На него сделали скидку {p}%. Сколько рублей будет стоить телефон после скидки? В ответ укажите одно число.",
                        f"Товар стоит {N} рублей. Скидка {p}%. Сколько рублей покупатель заплатит за товар после скидки? В ответ укажите одно число.",
                    ]),
                    "ans": str(final_price),
                })
